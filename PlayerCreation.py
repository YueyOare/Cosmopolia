...
class PlayerCreationGUI:
    def __init__(self):
        self.root = Tk()
        self.root.title("Створення гравців")
        ...
        self.label = Label(self.root, text="Введіть загальну кількість гравців:", bg="#B0B2FD", font=self.heading_font, fg="#010632")
        self.label.place(relx=0.5, rely=0.3, anchor=CENTER)

        self.entry = Entry(self.root, font=self.custom_font)
        self.entry.place(relx=0.5, rely=0.4, anchor=CENTER)

        self.button = Button(self.root, text="Далі", command=self.create_players, font=self.button_font, bg="#010632", fg="white", relief=RAISED)
    ...
    def create_player(self):
        ...
        self.player_label = Label(self.root, text=f"Введіть ім'я гравця {self.current_player}:", bg="#B0B2FD", font=self.heading_font, fg="#010632")
    ...
    def create_player_buttons(self):
        ...
        self.human_button = Button(button_frame, text="Гравець людина", command=lambda: self.add_player(True), font=self.button_font, bg="#010632", fg="white", relief=RAISED, bd=3, padx=10, pady=5, borderwidth=2, highlightthickness=0, activebackground="#010632")
        self.human_button.pack(side=LEFT, padx=10)

        self.bot_button = Button(button_frame, text="Гравець бот", command=lambda: self.add_player(False), font=self.button_font, bg="#010632", fg="white", relief=RAISED, bd=3, padx=10, pady=5, borderwidth=2, highlightthickness=0, activebackground="#010632")
    ...
    def display_players(self):
        ...
        players_text = "\n".join([f"Гравець {i + 1}: {player[0]} - {'Людина' if player[1] else 'Бот'}" for i, player in enumerate(self.Players)])
