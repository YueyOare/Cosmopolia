...
class MapGUI:
    ...
    def redraw_table(self, event=None):
        ...
        for i in range(config.fields_amount):
            if self.owners[i] is not None:
                x, y = self.circles_coords[i]
                self.canvas.create_oval(x, y, x + 2 * self.circle_radius, y + 2 * self.circle_radius,
                                        outline=config.players_colours[self.owners[i]],
                                        width=2)
    ...
    def planet_action(self):
        self.show_players()
        answer = self.map.array_Fields_in_map[self.players_positions[self.current_player]].event(
