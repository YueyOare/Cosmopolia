...
class System():
    ...
    class Planet(Field):  # клас нерухомості
        def __init__(self, name_of_planet="earth", price_buy=13, price_pay=648, owner=0, amount_of_branches=0,
                     branches="супутники", cost_of_branches=1993, rent_increase=30):
            super().__init__()
        ...
        def print_planet(self):
            if self.__owner == 0:
                print(self.__name_of_planet, self.__price_buy, self.__price_pay, self.__amount_of_branches,
                      "none owner")
            else:
                print(self.__name_of_planet, self.__price_buy, self.__price_pay, self.__amount_of_branches,
                      self.__owner.get_name())
